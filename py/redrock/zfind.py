...
def _mp_fitz(chi2, target_data, t, nminima, qout, archetype, use_gpu,new_penalty=False):
    ...
    try:
        ...
        for i, tg in enumerate(target_data):
            zfit = fitz(chi2[i], t.template.redshifts, tg,
                t.template, nminima=nminima, archetype=archetype, use_gpu=use_gpu,new_penalty=new_penalty)
...
def zfind(targets, templates, mp_procs=1, nminima=3, archetypes=None, priors=None, chi2_scan=None, use_gpu=False,new_penalty=False):
    ...
    Args:
        ...
        new_penalty (bool): variable to choose penalty versions
